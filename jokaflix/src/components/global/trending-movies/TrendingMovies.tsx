...
export default function TrendingMovies() {
  ...
  const { data, loading } = useFetch<{ results: TrendingMovie[] }>(
    {
      url: `https://api.themoviedb.org/3/trending/movie/day?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=${page}`,
    },
    { enabled: drawerOpen }
  );
  ...
  // Initial fetch for gallery (first 7 movies)
  const { data: initialData, loading: initialLoading } = useFetch<{ results: TrendingMovie[] }>({
    url: `https://api.themoviedb.org/3/trending/movie/day?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&append_to_response=credits,external_ids,videos,images`,
  });
          ...
          <DrawerContent className="app-drawer mx-auto w-full overflow-hidden px-4 py-6 xl:px-12" style={{ maxHeight: "90vh" }}>
            ...
            >
              ...
              {!hasMore && allMovies.length === 0 && (
                <div className="text-center text-gray-400 col-span-full">No trending movies found.</div>
              )}
            </div>
            {isFetchingNext && (
