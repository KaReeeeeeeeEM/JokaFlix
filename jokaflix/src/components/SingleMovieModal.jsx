...
export default function MovieModal({ toggler, title, type, movieId, onClose }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=${process.env.REACT_APP_TMDB_API_KEY}&query=${searchParam}`
      );
  ...
  const searchMovie = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/multi?api_key=${process.env.REACT_APP_TMDB_API_KEY}&query=${searchParam}`
      );
                      ...
                      {openDownloadModal && (
                        ...
                        {moviesByCategory.map((result) => (
                          <div key={result.id} className='w-screen md:w-full h-auto flex flex-col mx-2'>
                            ...
                            <div className='flex flex-col z-10 justify-center items-start text-left'>
