...
const Home = () => {
  ...
  const steps = [
    ...
    {
      target: '.watchlist',
      content: 'Want to keep track of your movies? Why dont you add them to your watch list.',
    },
    {
        ...
        <div className='overflow-y-auto bg-gray-900'>
         {isVisible && <Joyride
          ...
          {openQRCode && (
           ...
           {openWatchlist && (
            <Watchlist
              toggler={openWatchlist}
              onClose={() => setOpenWatchlist(false)}
            />
          )}
          <div className="absolute top-0 h-screen w-full inset-0 bg-opacity-70 bg-black blur-md"></div>
          <div className='w-full h-screen bg-gray-900' style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original${popularMovies[coverMovie].poster_path || popularMovies[coverMovie].backdrop_path})`, backgroundSize: "cover", backgroundRepeat: "no-repeat" }}>
            ...
            <div className='w-full h-4 px-8 my-6 flex justify-between items-center absolute top-0 right-0 z-30'>
              ...
              <div className='flex items-center justify-between w-[4rem] lg:w-[6rem] mr-4'>
                ...
                <button onClick={() => setOpenWatchlist(true)}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="orange" viewBox="0 0 24 24" stroke-width="1.5" stroke="orange" class="watchlist size-6 absolute rounded-full w-6 h-6 md:w-8 md:h-8 mr-8">
